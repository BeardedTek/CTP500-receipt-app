...
import type { PrintersCatalogLoadState } from '../config/usePrintersCatalog';
...
function BluetoothPanel({
  ...
  const printersReady = printersCatalog.status === 'ready';
  ...
  return (
    <div className="border rounded-lg p-4">
      ...
      <div className="flex gap-2 mb-2">
        {connection === 'connected' ? (
          ...
          <button
            type="button"
            onClick={onScan}
            disabled={connection === 'scanning' || !isSupported || !printersReady}
      ...
      {printersCatalog.status === 'loading' && (
        <p className="mt-2 text-sm text-gray-600">Loading printer definitions…</p>
      )}
      {printersCatalog.status === 'error' && (
        <div className="mt-3 rounded-md border border-red-200 bg-red-50 p-3 text-sm text-red-950">
          <p className="font-medium">Printer list failed to load</p>
          <p className="mt-1.5 leading-snug text-[0.8125rem] opacity-95">{printersCatalog.message}</p>
        </div>
      )}
      {!isSupported && (
        <div
  ...
  return (
    <div className="mx-auto w-full max-w-lg space-y-4 p-4 lg:max-w-5xl">
      <div className="flex flex-wrap items-center justify-center gap-3">
        ...
        <Link to="/printer-setup" className="text-sm text-blue-600 hover:underline">
          Add printer (BLE discovery)
        </Link>
      </div>
      ...
      <BluetoothPanel
        ...
        printersCatalog={printersCatalog}
        onScan={startScan}
